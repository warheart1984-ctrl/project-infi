"""Lab ledger — monotonic append-only project/session history."""
...
from lab.common import DEFAULT_LEDGER_PATH
...
def find_ledger_entries(
    ...
    *,
    ...
    matches = [item for item in rows if item.get("project_id") == project_id]
    if event:
        matches = [item for item in matches if item.get("event") == event]
    return matches
