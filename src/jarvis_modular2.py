...
class ProviderPayloadModule(ContextModule):
    ...
    def build(self, state: AssemblyState) -> Optional[ModuleOutput]:
        # This module is handled specially in assemble_provider_messages().
        # It does not independently emit a regular context payload here.
        return ModuleOutput(
            name=self.name,
...
def build_protocol_view(
    ...
    """
    Entry point for /api/jarvis/protocol?session_id=...

    This returns the inspectable modular view you described:
    - context_modules
    - modules
    - provider_messages
    - provider_preview
...
def make_runner_state(
    *,
    ...
    """
    Small helper for the agent/workspace runner.

    Important:
    The runner should emit structured state here, not mutate provider messages
    directly.
    """
    result: Dict[str, Any] = {}
...
def merge_workspace_context(
    ...
    """
    Safe merge for workspace context and runner state.
    """
...
def from_api_inputs(
    *,
    ...
    """
    Convenience adapter so api.py can pass plain dicts/lists.
    """
...
if __name__ == "__main__":
    ...
    protocol_payload = build_protocol_view(demo_state)
    print("=== Protocol View Helper ===")
    print(compact_jsonish(protocol_payload))
