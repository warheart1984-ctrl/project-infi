...
def _ensure_aris_path() -> None:
    aris_root = Path(__file__).resolve().parent.parent / "aris"
    aris_text = str(aris_root)
    if aris_text not in sys.path:
        sys.path.insert(0, aris_text)
...
class AAISULSubstrate:
    ...
    def _command_engine(self) -> Any:
        if self._command_runtime is None:
            _ensure_aris_path()
            from ul_substrate import SubstrateRuntime
    ...
    def status_payload(self) -> dict[str, Any]:
        ...
        return {
            ...
            "command_substrate": "aris.ul_substrate.ForgeGate",
            "identity_source": "UL",
