...
class DarzKernelBridgeAdapter(ULAdapter):
    name = "darz_kernel_bridge_adapter"

    def supports(self, raw: Any) -> bool:
        return (
            isinstance(raw, dict)
            and raw.get("bridge_id") == "darz.kernel.bridge"
            and raw.get("bridge_version")
        )

    def adapt(self, raw: Any) -> ULPayload:
        return ULPayload(
            source=self.name,
            kind="darz_kernel_bridge",
            section="protocol_trace",
            data={
                "bridge_version": raw.get("bridge_version"),
                "bridge_hash": raw.get("bridge_hash"),
                "accepted": raw.get("accepted"),
                "event_count": raw.get("event_count"),
                "invariant_count": raw.get("invariant_count"),
            },
            metadata={
...
def build_default_registry() -> ULRegistry:
    ...
    registry.register(DarzKernelBridgeAdapter())
