...
def _validate_transfer_request(
    *,
    ...
    earned_balance = float(sender_profile.earned_rail_credits or sender_profile.rail_credits or 0)
    ...
    if earned_balance < amount_f + fee:
        return {
            "status": "rejected",
...
def transfer_rail_credits(
    *,
    ...
    sender_profile.earned_rail_credits = max(
        0.0, float(sender_profile.earned_rail_credits or sender_profile.rail_credits or 0) - total_debit
    )
    sender_profile._sync_total_credits()
    sender_profile.updated_at = now
    recipient_profile.earned_rail_credits = max(
        0.0, float(recipient_profile.earned_rail_credits or 0) + amount_f
    )
    recipient_profile._sync_total_credits()
