...
DEFAULT_POLICY: dict[str, Any] = {
    ...
    "purchase": {
        ...
        "max_per_operator_per_day": 200,
        "admin_scope_required": "ugr:credits:purchase",
        "spend_without_reputation_floor": True,
    },
    "workflow": {"reputation": 12, "rail_credits": 2},
    "workflow_library": {"reputation": 20, "rail_credits": 4},
    "organ": {"reputation": 25, "rail_credits": 5},
    "organ_promotion": {"reputation": 30, "rail_credits": 6},
    "proof": {"reputation": 18, "rail_credits": 2},
    "trust_bundle": {"reputation": 22, "rail_credits": 3},
    "invariant": {"reputation": 14, "rail_credits": 2},
    "capability": {"reputation": 8, "rail_credits": 1},
    "capability_module": {"reputation": 16, "rail_credits": 3},
    "substrate": {"reputation": 10, "rail_credits": 2},
    "substrate_envelope": {"reputation": 5, "rail_credits": 1},
}
...
def purchase_policy(policy: dict[str, Any] | None = None) -> dict[str, Any]:
    return dict((policy or load_reward_policy()).get("purchase") or DEFAULT_POLICY.get("purchase") or {})


def purchase_enabled(policy: dict[str, Any] | None = None) -> bool:
    return bool(purchase_policy(policy).get("enabled", True))


def max_per_purchase(policy: dict[str, Any] | None = None) -> float:
    return float(purchase_policy(policy).get("max_per_purchase") or 100)


def max_purchase_per_operator_per_day(policy: dict[str, Any] | None = None) -> float:
    return float(purchase_policy(policy).get("max_per_operator_per_day") or 200)
