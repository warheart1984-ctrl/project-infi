...
export function usageText(): string {
  return [
    'image-studio — free cloud image studio (text-to-image + image-to-image)',
    '',
    'Usage:',
    '  image-studio -p "a red fox in snow" [--provider pollinations|gemini] [options]',
    ...
    '',
    'Options:',
    '  -p, --prompt <text>   Required. Generation or transformation prompt.',
    '  -m, --model <name>    Model to use (default: sana for pollinations; gemini-2.5-flash-image for gemini).',
    '  --provider <name>     pollinations (free, keyless) | genblaze (MRS stills, needs server) | storyforge (Infinity audio pipeline, needs server) | hfspace (free, keyless) | cloudflare | huggingface.',
    '  -o, --out <path>      Output file path (default: image-studio-output-<timestamp>.<ext>).',
    '  --url <image-url>     Optional reference image URL (image-to-image).',
    '  --width <px>          Output width (provider-dependent).',
    '  --height <px>         Output height (provider-dependent).',
    '  --seed <n>            Seed for reproducible output (when supported).',
