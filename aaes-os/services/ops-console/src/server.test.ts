...
describe('GET /telemetry', () => {
  ...
  it('surfaces SkillzMcGee continuity receipts for Nova Studio', async () => {
    const previousLedgerPath = process.env.SKILLZMCGEE_LEDGER_PATH;
    const tempDir = mkdtempSync(path.join(tmpdir(), 'skillz-ledger-'));
    const ledgerPath = path.join(tempDir, 'receipts.jsonl');
    const receipts = [
      {
        id: 'run-1',
    ...
    writeFileSync(ledgerPath, `\uFEFF${receipts.map((receipt) => JSON.stringify(receipt)).join('\n')}\n`, 'utf8');
    process.env.SKILLZMCGEE_LEDGER_PATH = ledgerPath;

    try {
      const response = await fetch(`${baseUrl}/skillzmcgee/ledger`);
      expect(response.status).toBe(200);
      const body = (await response.json()) as {
        source: string;
        available: boolean;
        receiptCount: number;
        state: Record<string, { last_status: string; last_run_id: string; last_output: unknown }>;
        recentReceipts: { id: string; slice: string; status: string }[];
        capabilities: { name: string; governed: boolean; receiptRequired: boolean }[];
      };

      expect(body.source).toBe(ledgerPath);
      expect(body.available).toBe(true);
      expect(body.receiptCount).toBe(2);
      expect(body.state.slice_math).toEqual({
        last_status: 'ok',
        last_output: { value: 42 },
        last_run_id: 'run-1',
      });
      expect(body.state['llm:slice_math'].last_output).toEqual({ text: 'state says 42' });
      expect(body.recentReceipts.map((receipt) => receipt.id)).toEqual(['run-2', 'run-1']);
      expect(body.capabilities.map((capability) => capability.name)).toEqual([
        'read_file',
        'write_file',
        'run_slice',
        'ask_llm',
      ]);
      expect(body.capabilities.every((capability) => capability.governed && capability.receiptRequired)).toBe(true);
    } finally {
      if (previousLedgerPath === undefined) {
        delete process.env.SKILLZMCGEE_LEDGER_PATH;
      } else {
        process.env.SKILLZMCGEE_LEDGER_PATH = previousLedgerPath;
      }
      rmSync(tempDir, { recursive: true, force: true });
    }
  });
