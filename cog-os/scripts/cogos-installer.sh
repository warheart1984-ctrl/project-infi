...
source "$INSTALLER_DIR/lib/installer/common.sh"
# shellcheck source=lib/installer/disk.sh
source "$INSTALLER_DIR/lib/installer/disk.sh"
# shellcheck source=lib/installer/copy.sh
source "$INSTALLER_DIR/lib/installer/copy.sh"
# shellcheck source=lib/installer/bootloader.sh
source "$INSTALLER_DIR/lib/installer/bootloader.sh"
# shellcheck source=lib/installer/identity.sh
source "$INSTALLER_DIR/lib/installer/identity.sh"
# shellcheck source=lib/installer/network.sh
source "$INSTALLER_DIR/lib/installer/network.sh"
# shellcheck source=lib/installer/firstboot.sh
source "$INSTALLER_DIR/lib/installer/firstboot.sh"

usage() {
  cat <<'USAGE'
Usage:
  bash cog-os/scripts/cogos-installer.sh [options]

Modes:
  Default is plan-only (no disk writes). Use --apply for actual install.

Options:
  --target-disk /dev/sdX|/dev/nvme0n1   Install target disk (--disk alias)
  --rootfs /path/to/rootfs              Rootfs source (default: COGOS_ROOTFS_SRC)
  --hostname NAME                       Installed hostname (default: nova-cog-os)
  --user NAME                           Installed user (default: cogos)
  --password PASS                       Optional password for installed use
...
ROOTFS_SOURCE="${COGOS_ROOTFS_SRC:-$COGOS_ROOTFS_OUT}"
INSTALL_HOSTNAME="${COGOS_INSTALL_HOSTNAME:-nova-cog-os}"
INSTALL_USER="${COGOS_INSTALL_USER:-cogos}"
INSTALL_PASSWORD="${COGOS_INSTALL_PASSWORD:-}"
INSTALLER_VERSION="${COGOS_INSTALLER_VERSION:-1.0.0}"
INJECT_FAIL_STEP="${COGOS_INSTALLER_FAIL_STEP:-}"

while [[ $# -gt 0 ]]; do
  case "$1" in
    --target-disk|--disk) TARGET_DISK="${2:-}"; shift 2 ;;
...
if [[ -z "$TARGET_DISK" ]]; then
  if command -v whiptail >/dev/null 2>&1; then
    ...
    TARGET_DISK="$(whiptail --title 'CoG OS Installer' --menu 'Select target disk' 20 80 10 "${menu_args[@]}" 3>&1 1>&2 2>&3)" || die "Disk selection cancelled."
  else
    log "Available disks:"
...
if [[ "$ASSUME_YES" != "1" ]]; then
  if command -v whiptail >/dev/null 2>&1; then
    whiptail --title 'CoG OS Installer' --yesno "Install to $TARGET_DISK?\nThis will erase the disk." 12 70 || die "Install cancelled."
  else
