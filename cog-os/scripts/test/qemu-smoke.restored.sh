...
materialize_rootfs_disk() {
  ...
  if [[ -f "$img" ]]; then
    ...
    if [[ -f "$rootfs_stamp" && "$img" -nt "$rootfs_stamp" ]]; then
...
run_contract_boot() {
  ...
  if [[ ! -f "$initrd" ]]; then
    # Debian names initrd initrd.img-$KVER (not initrd.img); plain initrd*.img misses that suffix.
  ...
  # Root disk on IDE (PIIX): minimal Debian initramfs loads PIIX/IDE via init-top/all_generic_ide
  # but often lacks local-top hooks to modprobe virtio_blk before root mount.
  local -a qemu_cmd=(
  ...
  if [[ -n "$initrd" && -f "$initrd" ]]; then
    if [[ "${COG_QEMU_USE_INITRD:-1}" != "0" ]]; then
      use_initrd=1
    fi
  fi
  ...
  if [[ -n "$kernel" && -f "$kernel" ]]; then
    ...
    if [[ "$use_initrd" -eq 1 ]]; then
      ...
      log "boot: using initrd $initrd"
    else
      log "boot: direct kernel boot (no initrd; install initrd.img under $ROOTFS/boot or set COG_QEMU_USE_INITRD=1)"
    fi
    qemu_cmd+=(-append "console=ttyS0 root=/dev/sda rw init=/sbin/init profile=$PROFILE")
  else
