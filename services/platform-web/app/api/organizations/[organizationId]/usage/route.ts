...
export async function GET(request: Request, { params }: { params: Promise<{ organizationId: string }> }) {
  ...
  const response = await fetch(`${getPlatformApiBaseUrl()}/v1/organizations/${encodeURIComponent(organizationId)}/usage`, {
    headers: { 'x-session-id': sessionId },
  });
  const payload = await response.json().catch(() => ({ error: 'organization usage lookup failed' }));
