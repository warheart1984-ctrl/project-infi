"""Tests for governed civilization observation (GCV-0)."""
...
class GovernedCivilizationObserveTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_registry_valid(self):
        self.assertEqual(validate_civilization_registry(repo_root=Path(self._repo_tmp.name)), [])

    def test_observe_without_overlay_write(self):
        result = self.runtime.observe_civilization_drift(window_days=30)
        self.assertEqual(result.get("outcome"), "observed")
        self.assertEqual(result.get("gcv_class"), "GCV-0")
        self.assertFalse(self.runtime._overlay_path.is_file())

    def test_api_get(self):
        res = self.client.get("/api/operator/civilizations")
