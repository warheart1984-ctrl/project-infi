#!/usr/bin/env python3
"""Promote Alt-19 organs from MVP to governed."""
...
ELIGIBILITY = _ROOT / "tools/governance/check_alt19_governed_eligibility.py"
...
def main() -> int:
    ...
    for gene in ALT19_GENES:
        if (_load(gene).get("identity") or {}).get("stage") == "governed":
            print(f"[alt19-governed] {gene} already governed")
            continue
        ...
        if not decision.passed or decision.target_stage != "governed":
            print(f"[alt19-governed] {gene} blocked: {decision.failures}")
            return 1
        decision = engine.apply(decision)
        if not decision.passed:
            return 1
        print(f"[alt19-governed] {gene} -> governed")
    return 0
