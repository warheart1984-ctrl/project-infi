#!/usr/bin/env python3
"""Promote Alt-15 organs from MVP to governed."""
...
ALT15_GOVERNED = {
    gene: f"docs/proof/nova/{gene.upper()}_GOVERNED_PROOF.md"
    for gene in (
...
ELIGIBILITY = _ROOT / "tools/governance/check_alt15_governed_eligibility.py"
...
def main() -> int:
    ...
    if proc.returncode != 0:
        print("[alt15-governed] eligibility gate failed")
        return 1

    engine = PromotionEngine(_ROOT)
    for gene, proof in ALT15_GOVERNED.items():
        current = (_load(gene).get("identity") or {}).get("stage", "")
        if current == "governed":
            print(f"[alt15-governed] {gene} already governed")
            continue
        ...
        if not decision.passed or decision.target_stage != "governed":
            print(f"[alt15-governed] {gene} blocked: {decision.failures}")
            return 1
        decision = engine.apply(decision)
        if not decision.passed:
            print(f"[alt15-governed] {gene} apply failed: {decision.failures}")
            return 1
        print(f"[alt15-governed] {gene} -> governed")
    return 0
