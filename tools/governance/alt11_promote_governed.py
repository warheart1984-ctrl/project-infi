#!/usr/bin/env python3
"""Promote Alt-11 organs from MVP to governed."""
...
ALT11_GOVERNED = {gene: f"docs/proof/platform/{gene.upper()}_GOVERNED_PROOF.md" for gene in (
...
ELIGIBILITY = _ROOT / "tools/governance/check_alt11_governed_eligibility.py"
...
def main() -> int:
    ...
    if proc.returncode != 0:
        print("[alt11-governed] eligibility gate failed")
        return 1

    engine = PromotionEngine(_ROOT)
    for gene, proof in ALT11_GOVERNED.items():
        ...
        if not decision.passed or decision.target_stage != "governed":
            print(f"[alt11-governed] {gene} blocked: {decision.failures}")
            return 1
        decision = engine.apply(decision)
        if not decision.passed:
            print(f"[alt11-governed] {gene} apply failed: {decision.failures}")
            return 1
        print(f"[alt11-governed] {gene} -> governed")
    return 0
