#!/usr/bin/env python3
"""Promote Release 20 subsystems from MVP to governed."""
...
ELIGIBILITY = _ROOT / "tools/governance/check_alt20_governed_eligibility.py"
...
def main() -> int:
    ...
    for gene in ALT20_GENES:
        if (_load(gene).get("identity") or {}).get("stage") == "governed":
            print(f"[alt20-governed] {gene} already governed")
            continue
        ...
        if not decision.passed or decision.target_stage != "governed":
            print(f"[alt20-governed] {gene} blocked: {decision.failures}")
            return 1
        decision = engine.apply(decision)
        if not decision.passed:
            return 1
        print(f"[alt20-governed] {gene} -> governed")
    return 0
