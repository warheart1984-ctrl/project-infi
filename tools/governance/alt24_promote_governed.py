#!/usr/bin/env python3
"""Promote Release 24 subsystems from MVP to governed."""
...
GOVERNED_PROOFS = {
    gene: f"docs/proof/platform/{gene.upper()}_GOVERNED_PROOF.md" for gene in ALT24_GENES
}

ELIGIBILITY = _ROOT / "tools/governance/check_alt24_governed_eligibility.py"
...
def main() -> int:
    ...
    for gene in ALT24_GENES:
        if (_load(gene).get("identity") or {}).get("stage") == "governed":
            print(f"[alt24-governed] {gene} already governed")
            continue
        ...
        if not decision.passed or decision.target_stage != "governed":
            print(f"[alt24-governed] {gene} blocked: {decision.failures}")
            return 1
        decision = engine.apply(decision)
        if not decision.passed:
            return 1
        print(f"[alt24-governed] {gene} -> governed")
    return 0
